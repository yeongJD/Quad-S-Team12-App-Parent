...
- 실제 판정은 `TimePolicy` 존재 여부와 자녀 제출 산출물인 `WeeklyBudget`/`WeeklyTimeDistribution` 존재 여부를 분리해서 본다.
...
| 자녀 계획 존재 여부 | backend weekly budgets + weekly templates | 없음 | `TimePolicy`만 있으면 아직 자녀 계획 전 상태 |
...
3. "오늘의 시간"이 없는 상태의 기준은 무엇인가요?
   ...
   - 구현 기준: 부모 제출 `TimePolicy`만 있으면 `waitingChildPlan`, 자녀 제출 `WeeklyBudget`/`WeeklyTimeDistribution`까지 있으면 `hasChildPlan`으로 본다.
